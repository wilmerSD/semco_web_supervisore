...
import 'package:app_tasking/infrastructure/models/response/response_list_subtask_model.dart';
...
class SubtaskProvider with ChangeNotifier {
  ...
  // VARIABLES
  Task subTaskObject = Task();
  List<DataListSubtask> listSubtask = [];
  List<Note> listNotesSubTask = [];
  List<Checklist> checkListSubTask = [];
  ...
  bool isShowCheckList = true;
  bool isShowListNotes = true;
  bool isShowHeaderSubTask = true;
  bool isGettingCheckSubTask = false;
  ...
  Future<void> getListNotes(
      [String? pTaskId, String? pClienteId, String? pCompaniaId]) async {
    isGetListNotes = true;
    listNotesSubTask = [];
    try {
      final response = await noteRepositoryImpl.getListNotes(
          pTaskId ?? "", pClienteId, pCompaniaId);

      if (response.isEmpty) {
        return;
      }
      listNotesSubTask.addAll(response);
    } catch (e) {
  ...
  Future<void> getCheckListSubTask(String pTaskId, String pCampanaId,
      String pClienteId, String pCompaniaId) async {
    isGettingCheckSubTask = true;
    checkListSubTask = [];
    ...
    try {
      ...
      checkListSubTask.addAll(response);
    } catch (e) {
    } finally {
      isGettingCheckSubTask = false;
      notifyListeners();
    }
  }
  ...
  void updateShowCheckList() {
    isShowCheckList = !isShowCheckList;
    notifyListeners();
  }

  void updateShowListNotes() {
    isShowListNotes = !isShowListNotes;
    notifyListeners();
  }

  void visibleHeaderSubTask() {
    isShowHeaderSubTask = !isShowHeaderSubTask;
    notifyListeners();
  }
}
